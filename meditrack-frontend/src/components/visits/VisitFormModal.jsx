  ...
  return (
    <Modal title={visit ? 'Edit Visit' : 'Log Doctor Visit'} onClose={onClose} size="lg">
      <div className="flex flex-col gap-4">
        ...
        <div>
          ...
          <input
            ...
            className={`w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${
              errors.doctorName ? 'border-red-400' : 'border-bg-border'
            }`}
          />
        ...
        {/* Visit date */}
        <div>
          <label className="block text-sm font-medium text-text-secondary mb-1">Visit Date</label>
          <input
            type="date"
            value={form.visitDate}
            onChange={e => setForm(f => ({ ...f, visitDate: e.target.value }))}
            className={`w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${
              errors.visitDate ? 'border-red-400' : 'border-bg-border'
            }`}
          />
        ...
        {/* Diagnosis */}
        <div>
          <label className="block text-sm font-medium text-text-secondary mb-1">Diagnosis (optional)</label>
          <input
            type="text"
        ...
        <div>
          ...
          {medications && medications.length > 0 ? (
                ...
                <label
                  ...
                  className={`flex items-center gap-2 p-2 rounded-lg border cursor-pointer text-sm ${
                    form.medicationIds.includes(m.id)
                      ...
                      : 'border-bg-border hover:border-bg-border'
                  }`}
                >
                  <input
                    type="checkbox"
            ...
            <p className="text-xs text-text-secondary">No active medications available to link.</p>
          )}
        </div>
